    ...
    try:
        if isinstance(x, torch.Tensor):
            ...
            if numel <= 1000:
                ...
                print(f"{name}: shape={shape}, dtype={dtype}, device={device}, values={vals}")
            else:
                sample = x.reshape(-1)[:20].detach().cpu().numpy()
                print(f"{name}: shape={shape}, dtype={dtype}, device={device}, numel={numel}, first_20={sample}")
        else:
            # for Python scalars or other types
...
class PI0Pytorch(nn.Module):
    ...
    def denoise_step(
        ...
        # npy_save_path = "/mnt/data/weiyang.hu/openpi/gemma_expert_npy_1024/"
        # save_kv_cache(past_key_values, npy_save_path)
        # _debug_print("state", state)
        # _debug_print("x_t", x_t)
        # _debug_print("timestep", timestep)
        # np.save(os.path.join(npy_save_path, "x_t.npy"), x_t.detach().cpu().numpy())
        # np.save(os.path.join(npy_save_path, "timestep.npy"), timestep.detach().cpu().numpy())
        # np.save(os.path.join(npy_save_path, "state.npy"), state.detach().cpu().numpy())
        suffix_embs, suffix_pad_masks, suffix_att_masks, adarms_cond = self.embed_suffix(state, x_t, timestep)
        # _debug_print("suffix_embs", suffix_embs)
        # _debug_print("adarms_cond", adarms_cond)
